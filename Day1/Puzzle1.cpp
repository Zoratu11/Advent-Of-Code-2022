//DAY 1 PUUZLE 1 OF ADVENT OF CODE
...
//This function finds the elf that is carrying the most food according to the prompt above (from a file) and returns how much food they are carrying
const int getMostCalories(const std::string& fileName);

//takes an elf file stream and splits it based on newline characters and adds the numbers in between the whitespace
const std::vector<int> splitAndAdd(const std::ifstream& fileStream);

//returns the highest number in a vector
const int getMax(const std::vector<int> vec);


int main()
{
    int calories = getMostCalories("input.txt");
    
    std::cout << "The most food that one elf had was: " << calories << std::endl;
...
const int getMax(std::vector<int> vec)
{
    int currentMax = 0;
